...
Things or Feautures that could be added 
1. Real time Financial anomaly detection. 
not just "here's your total" but an endpoint that flags suspicious or unusual patterns in the financial data automatically. 
Something like GET/dashboard/anomalies that returns: 
Expense in Travel category on March 15 was 74000 that's 2.4x the average for this category. Two duplicate looking entries detected in office supplies within 3 mins of each other. 
2. SMS powered auto ledger
the flow would be your friend's phone receives a bank UPI/SMS -> an andriod app or automation tool like (tasker or macrodroid) forwards it via webhook to the backend -> extract amount, merchant, type(credit/debit), date, auto creates a categorized, financial record, anomaly detection runs on it. dashboard updates in real time. 